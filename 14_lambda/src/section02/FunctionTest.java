...
public class FunctionTest {
	public static void main(String[] args) {
		...
		/* Function 계열
		 * - 매개변수를 받아서 다른 타입으로
		 *   변환(가공)하여 반환하느 함수형 인터페이스
		 * 
		 * - R applyXXX() 메서드 제공
		 */
		
		
		/* Function<T, R> : 매개변수 T 전달, R타입 반환 */
		Function<String, Integer> strLength = str -> str.length();
		
		System.out.println(strLength.apply("Function의 글자수"));
		
		/* BiFunction<T,U,R> : T와 U를 전달 받아 R형태로 반환 */
		BiFunction<Integer, Integer, Double> divTwoNumber= (a,b) 
				-> (double)a/b;
		...
		range -> Math.random() * range; // 0이상 range 미만 실수 반환
		Set<Double> set = new HashSet<Double>();
		while(set.size() < 10) { // set에 저장된 객체 수 10미만이면 반복
