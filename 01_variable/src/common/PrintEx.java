...
public class PrintEx {
   ...
   *   
   * [패턴 종류]
   * -%d (Decimal, 10진 정수) : 정수(byte, short, int, long)만 대입가능
   * 
   ...
   * ex) %4d, 3 대입 -> ㅁㅁㅁㅁ(4칸확보) 오른쪽끝에 3대입 --> ㅁㅁㅁ3
   *  
   * - 음의 정수 : 정수 크기만큼 칸 확보 + 왼쪽 정렬
   * ex) %-4d, 3 대입 -> ㅁㅁㅁㅁ(4칸확보) 왼쪽끝에 3대입 --> 3ㅁㅁㅁ
   * 
   * 
   * - 소수점 : 소수점 아래 몇번째 자리까지 표시할지 지정
   * 					(지정된 자리 아래에서 반올림 처리)
   * ex) $.1f, 3.15대입 -> 3.15에서 5반올림 처리 -> 3.2
   * 
   * 
   */
  	...
  	String name = "신짱구"; // String = 참조형(값저장 x, 값의 위치(주소)저장)
  	...
  	// println()버전
  	System.out.println(name +"는 " + age +"세 "+ gender +"아로" + " 키는 " + height + "cm이며 " + "자바공부여부 : "+ javaStudy);
  	
  	// 신짱구는 5세 남아로 키는 80.5cm이며, 자바 공부 여부: false
  	// printf()버전
  	System.out.printf("%s는 %d세 %c아로 키는 %.1fcm이며, 자바공부여부 : %b",name,age,gender,height,javaStudy);
