...
//	if(list.size() == 0) {
	...
	private void selectIndex() {
		...
		if(book == null) {
			System.out.println("해당 인덱스에 책의 정보가 존재하지 않습니다.");
			return;
		}
	...
	private void addBook() {
	  ...
	  // 엔터가 입력되기 전 까지의 문자열 얻어오기(띄어쓰기 포함)
...
//	}
	...
	private void removeBookTitle() {
		System.out.print("제거할 책 제목을 입력 : ");
		String title = sc.nextLine();
		
		// 제거 서비스 호출
		boolean book = service.removeBookTitle(title);
		if(book == false) {
			System.out.println("삭제할 책 제목이 없음");
		}else {
			System.out.printf("입력한 책이 제거되었습니다.");
	...
	private void selectPublisherAll() {
		...
		// 조회서비스 호출 (결과 개수 : 0~n개)
		List<BookDTO> box = service.selectPublisherAll(publisherName);
		
		//조회결과가 없을 경우
		if(box.isEmpty()) {// List에 저장된 객체가 없을 경우
			System.out.println("검색결과 없음");
			return;
		}
		for(BookDTO book : box ) {
			System.out.println(book);
		}
	}
	
	
	// 입력받은 저자가 일치하는 모든 책 조회하기
	private void selectAuthorAll() {
		System.out.println("저자 이름 입력 : ");
		String author = sc.nextLine();
		
		// 조회서비스 호출 (결과 개수 : 0~n개)
		List<BookDTO> box = service.selectAuthorAll(author);
	...
	private void searchBook() {
		...
		// 조회서비스 호출 (결과 개수 : 0~n개)
		List<BookDTO> box = service.searchBook(keyword);
