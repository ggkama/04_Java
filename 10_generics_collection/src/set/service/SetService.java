...
public class SetService {
	...
	public void test1() {
		...
		/* boolean contains(E e) : 동등 객체포함 여부 확인
		 * -> equals(), hashCode() 오버라이딩 필수 */
	 ...
	 * 
	 * - hashCode() : 객체의 식별번호 
	 *       equals()가 true면 hashCode()도 같아야함
	 *       -> 빠르게 찾기 위해 활용
	 */
	public void test2() {
		...
		/* Set의 특징 : 순서 유지 X, 중복데이터 허용 X */
		for(Person p : personSet) {
			System.out.println(p);
		}
		
		// 1.equal() 오버라이딩 x , hashCode() 오버라이딩 x 인경우
		// -> 중복제거 안됨
		// 2.equal() 오버라이딩 O , hashCode() 오버라이딩 x 인경우
		// -> 중복제거 안됨
		// 3.equal() 오버라이딩 X , hashCode() 오버라이딩 O 인경우
		// -> 중복제거 안됨
		
		// 4.equal() 오버라이딩 O , hashCode() 오버라이딩 O 인경우
		// -> 둘다 오버라이딩이 되어있어야 중복제거 가능

	}
	
	/* [Iterator<T> Set<T>.iterator()]
	 * 
	 * - 해당 Set에 저장된 요소를 순차접근할 수 있는
	...
	public void test3() {
		...
		// 반복 접근할 (꺼낼)객체가 존재하면 반복
		// 반복 접근할 객체가 존재하지 않으면 false
		while(it.hasNext()) {
	 ...
	 * 
	 * ** 선행 조건 **
	 * 저장되는 객체가 Comparable 인터페이스를 상속받아 구현
	 * -> add() 될 때 마다 자동으로 비교 -> 정렬을 진행
	 */
	public void test4() {
		
		// 컬랙션은 객체만 사용가능해서 Int 대신 Integer 사용
		// 순서를 저장하는 Set
//		Set<String> set = new HashSet<String>();
...
//			int num = random.nextInt(45+1); // 1부터 45사이 난수 생성
//			lotto[i] = num;
//			
//			for(int x=0; x<i ; x++) {
//				if(lotto[x] == num) {
...
//		return null;
	...
	public void lottoNumberGenerator() {
		...
		System.out.println("총 금액 : "+count*1000 + "원");
	}
